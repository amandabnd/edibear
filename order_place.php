...
// If no POST data, redirect back to checkout
...
// Generate order number based on payment method (easy to extend later)
$prefix = $paymentMethod === 'bank_transfer' ? 'B#' : 'C#';
$orderNumber = $prefix . date('ymdHis');
$createdAt = date('Y-m-d H:i:s');

// Create orders table suggestion:
// CREATE TABLE orders (
...
// Insert order (extendable for other payment methods via payment_status)
...
// Clear cart after placing order (for both methods for now)
$user->deleteTableRow("cart", array("session_id" => $session_id));
