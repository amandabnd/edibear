<?php
    ...
    $totalpdfPages = ceil( count($user->fetchAll(array("id"), array("pdf_details"), array("status"=>1), "", $searchTagLike . " AND " . $searchKeyLike)) / 16);
    if ( isset($_GET['page']) ) {
...
<body>
    ...
    <div class="container-fluid py-3" style="margin-top: 110px !important;">
        ...
        <div class="d-flex pageheaderdiv">
            ...
            <div class="col-lg-8 ">
                <i class="fa fa-home pt-1 pr-2 text-primary"></i><a href="./">Home</a><i class="fa fa-angle-right pt-1 px-2 text-primary"></i>Coloring Pages
            ...
            <?php
            $tagsArr = array();
            foreach ($user->fetchAll(array("tag"), array("pdf_details"), array("status" => 1)) as $row) {
            ...
            for ($i = 0; $i < $visibleTags; $i++) {
                $tag = $tagsArr[$i];
                echo "<a  style='color:#000;  border-color:#a7a7a7; border-style:solid; border-width: 1px;' class='tagfont px-3 py-1 mr-1 mb-2' href='./pdf?tag=$tag' class='btn btn-light m-1'>$tag</a>";
            }
            ...
            if ($totalTags > $visibleTags) {
                ...
                for ($i = $visibleTags; $i < $totalTags; $i++) {
                    $tag = $tagsArr[$i];
                    echo "<a  style='color:#000; border-color:#a7a7a7; border-style:solid;' class='tagfont px-3 py-1 mr-1 mb-2 mt-1' href='./pdf?tag=$tag' class='btn btn-light m-1'>$tag</a>";
                }
                echo "</div>";
                echo "</div>";
                    ...
                    <div class="row pb-3">
                        <?php
                            if ( $pdfPageNo > 1 ) {
                                ...
                                $other = "id<" . $user->fetchAll(array("id"), array("pdf_details"), array("status"=>"1"), "id DESC LIMIT $limit", $searchTagLike . " AND " . $searchKeyLike)[$limit-1]['id'];
                            } else {
                                $other = "";
                            }
                            if ( $other!="" ) {
                           ...
                           // card views have defined by using below code block. if you need to change the card count you may change the DESC LIMIT {number-you-need-to-show} 
                            foreach ( $user->fetchAll(array("id","tag","title","image", "description","timestamp", "pdfupload","download_count"), array("pdf_details"), array("status"=>"1"), "id DESC LIMIT 16", $other) as $row ) {
                                echo $widgets->displaypdfBrief($row, "col-md-3", 200, false);
                            }
                        ?>
