<?php
    ...
    $totalad1Pages = ceil( count($user->fetchAll(array("id"), array("ad1_details"), array("status"=>1), "", $searchTagLike)) / 6);
    if ( isset($_GET['page']) ) {
        $ad1PageNo = (int)$_GET['page'];
        if ( $totalad1Pages < $ad1PageNo ) {
            $user->redirect("./ad1?page=$totalad1Pages");
        }
    } else {
        $ad1PageNo = 1;
    }
...
<body>
    ...
    <div class="container-fluid py-3">
        <div class="container pt-3">
            <i class="fa fa-home pt-1 pr-2 text-primary"></i><a href="./">Home</a><i class="fa fa-angle-right pt-1 px-2 text-primary"></i>ad1
            <h4 class="text-warning mt-2">Fun Activities</h4>

            <?php 
                //echo $widgets->displayGetQuoate($user);
                $tagsArr = array();
                foreach ( $user->fetchAll(array("tag"), array("ad1_details"), array("status"=>1), "", "", "","","", ) as $row ) {
                ...
                <div class="d-flex flex-wrap m-n1">
                    <?php
                        foreach ( $tagsArr as $tag ) {
                            echo "<a href='./ad1?tag=$tag' class='btn btn-light m-1'>$tag</a>";
                ...
                <div class="col-lg-9">
                    <div class="row pb-3">
                        <?php
                            if ( $ad1PageNo > 1 ) {
                                $limit = ( $ad1PageNo - 1 ) * 6;
                                $other = "id<" . $user->fetchAll(array("id"), array("ad1_details"), array("status"=>"1"), "id DESC LIMIT $limit", $searchTagLike)[$limit-1]['id'];
                            } else {
                                $other = "";
                            }
                            if ( $other!="" ) {
                            ...
                            foreach ( $user->fetchAll(array("id","tag","title","image", "description","timestamp"), array("ad1_details"), array("status"=>"1"), "id DESC LIMIT 6", $other) as $row ) {
                                echo $widgets->displayad1Brief($row, "col-md-6", 200, false);
                            }
                        ?>
                        <div class="col-12">
                            <?php
                                ...
                                if ( $ad1PageNo == $totalad1Pages ) {
                                    $nextBtn = "disabled";
                                } 
                                echo "
                                    <nav aria-label='Page navigation example'>
                                        <ul class='pagination justify-content-center'>
                                            <li class='page-item mx-1 $previousBtn' onclick='prevNextBtn(0, $ad1PageNo, $totalad1Pages)'>
                                                <a class='page-link' href='#0' aria-label='Previous'>
                                                    <span aria-hidden='true'>&laquo;</span>
                                                </a>
                                            </li>";
                                            for ( $i=1; $i<=$totalad1Pages; $i++ ) {
                                                if ( $i == $ad1PageNo ) {
                                                    echo "<li class='page-item active mx-1'><a class='page-link' href='#'>$i</a></li>";
                                                } else {
                                                    echo "<li class='page-item mx-1'><a class='page-link' href='./ad1?page=$i$pagingUrlParm'>$i</a></li>";
                                                }
                                            }
                                echo "
                                            <li class='page-item mx-1 $nextBtn' onclick='prevNextBtn(1, $ad1PageNo, $totalad1Pages)'>
...
<div style="display:flex; justify-content:space-around;" class="mt-5 mb-5">
    <div style="background-color: #a1a1a1; height: 420px; width: 70%; display:flex; align-items:center; justify-content:space-around;">
        <h1 class="text-center"> Advertiesment </h1>
    </div>
</div>
    ...
    <script>
        ...
        function prevNextBtn(btn, ad1Page, totalPages) {
            ...
            if ( btn == 0 ) {
                if ( ad1Page > 1 ) {
                    location.href = "./ad1?page=" + (ad1Page-1) + searchTag;
                }
            } else if ( btn == 1 ) {
                if ( totalPages > ad1Page ) {
                    location.href = "./ad1?page=" + (ad1Page+1) + searchTag;
                }
            }
        }
