  ...
  if ( $user->is_loggedin() ) {
    if ( $user->checkTimeout() ) {
      if ( isset($_POST['newUserSubmit']) || isset($_POST['editUserSubmit']) || isset($_POST['deleteUserSubmit']) ){
        $inputUserUsername = htmlspecialchars(isset($_POST['inputUserUsername']) ? $_POST['inputUserUsername'] : "");
        $inputUserEmail = htmlspecialchars(isset($_POST['inputUserEmail']) ? $_POST['inputUserEmail'] : "");
        $inputUserCountry = htmlspecialchars(isset($_POST['inputUserCountry']) ? $_POST['inputUserCountry'] : "");
        ...
        if ( isset($_POST['newUserSubmit']) || isset($_POST['editUserSubmit']) ){
          if ( $inputUserPassword == $inputUserConfirmPassword ) {
            $inputUserPassword = "pass".password_hash($inputUserPassword, PASSWORD_DEFAULT);
            if ( isset($_POST['newUserSubmit']) ) {
              $user->insertTable("tourists", array("username"=>$inputUserUsername, "password"=>$inputUserPassword, "email"=>$inputUserEmail, "country"=>$inputUserCountry));
              ...
              $user->updateTable("tourists", array("username"=>$inputUserUsername, "password"=>$inputUserPassword, "email"=>$inputUserEmail, "country"=>$inputUserCountry), array("id"=>$UserHiddenID));
          ...
          $UserHiddenID = (int)isset($_POST['UserHiddenID']) ? $_POST['UserHiddenID'] : "";
          $deleteTouristSubmit = $user->confirmDeleteModal($UserHiddenID, $inputUserUsername, $inputUserEmail, "Confirm Delete a Tourist", "manage-users");
        } 
      } else if ( isset($_POST['confirmDeleteSubmit']) ) {
        $deleteNameID = (int)isset($_POST['deleteNameID']) ? $_POST['deleteNameID'] : "";
        $user->updateTable("tourists", array("status"=>0, "delete_status"=>1), array("id"=>$deleteNameID));
...
<body class="g-sidenav-show   bg-gray-100">
    ...
    <div class="container-fluid py-4">
      ...
      <?php 
        ...
        if ( isset($_POST['confirmDeleteUserSubmit']) ) {
          ...
          $stmtDeleteUser = $connection->prepare("UPDATE `tourists` SET `delete_status`='1' WHERE `id`=? ");
  ...
  <script>
    var passwordStatus = usernameStatus = false;
    ...
    $("input[name='inputUserUsername']").keyup(function () {
      $.ajax({
        ...
        url: "ajax.php",
        data: {
          checkTouristUsername: $("input[name='UserHiddenID']").val(),
